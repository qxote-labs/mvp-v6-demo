...
(async () => {
  ...
  try {
    ...
    console.log('3) 카마스터: 담당 고객 목록 건수 확인 (2건이어야 함):', countBefore);
    ...
    await karmaster.click('button[id^="km-need-n-"]');
    ...
    // 요청 가능 상태로 이어지지 않는다).
    const pass = countBefore === 2 && stillWaitingBadge.includes('고객요청')
