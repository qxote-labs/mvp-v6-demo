...
(async () => {
  ...
  try {
    ...
    // 이제 "신차 케어 서비스" 카드가 접혀 있어 먼저 펼쳐야 버튼이 보인다.
    await customer.waitForSelector('summary:has-text("신차 케어 서비스")', { timeout: 3000 });
    await customer.click('summary:has-text("신차 케어 서비스")');
